...
import numpy as np
from matchnet.code.utils.misc import largest_cc,mask2bbox
...
def placement_background_substract(c_height_i,d_height_i,c_height,d_height):
    _, img_otsu = cv2.threshold(d_height_i,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    bool_otsu = largest_cc(img_otsu)#largest_cc返回值是一个bool类型的矩阵
    # mask_kit = np.zeros_like(d_height_i)
    # rmin,rmax,cmin,cmax = mask2bbox(bool_otsu)
    # mask_kit[rmin:rmax,cmin:cmax] = 1
    c_height_i = np.where(bool_otsu, c_height_i, 0)
    d_height_i = np.where(bool_otsu, d_height_i, 0)
    #同样对盒子分割
    _, img_otsu = cv2.threshold(d_height,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    bool_otsu = largest_cc(img_otsu)#largest_cc返回值是一个bool类型的矩阵
    # mask_kit = np.zeros_like(d_height_i)
    # rmin,rmax,cmin,cmax = mask2bbox(bool_otsu)
    # mask_kit[rmin:rmax,cmin:cmax] = 1
    c_height = np.where(bool_otsu, c_height, 0)
    d_height = np.where(bool_otsu, d_height, 0)

    return c_height_i,d_height_i,c_height,d_height


def get_kit(c_height,d_height):
    _, img_otsu = cv2.threshold(d_height,0,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    bool_otsu = largest_cc(img_otsu)#largest_cc返回值是一个bool类型的矩阵
    # mask_kit = np.zeros_like(d_height_i)
    # rmin,rmax,cmin,cmax = mask2bbox(bool_otsu)
