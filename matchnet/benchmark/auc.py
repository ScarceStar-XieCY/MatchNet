...
if __name__ == "__main__":
    ...
    parser.add_argument("--pkl_files", default=["ORB-PE_acc.pkl","bear_acc.pkl"], nargs='+', type=str)
    ...
    for method_name, method_dict in zip(names, accs):
        ...
        for k, v in method_dict.items():
            ...
            for acc_list, acc_name, x_name in zip(acc_lists, acc_names, x_names):
                ...
                accuracies = []
                if acc_name == "err_rot":
                    thresholds = np.linspace(0, MAX_DEG_THRESH, 30)
                elif acc_name == "err_reproj":
                    thresholds = np.linspace(0, MAX_PIX_THRESH, 20)
                elif acc_name == "err_tr":
                    thresholds = np.linspace(0, MAX_TR_THRESH, 20)
                else:
                    thresholds = np.linspace(0, MAX_LENGTH_THRESH, 20)
    ...
    fig.tight_layout()
    plt.savefig(os.path.join(plot_dir, "4x4_2.pdf"), format="pdf", dpi=200)
    plt.show()
